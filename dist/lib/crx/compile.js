...
export async function generateCrx(args) {
    if (mutex.isLocked()) {
    ...
    await mutex.runExclusive(async () => {
        ...
        console.log(`crx generated at: ${crxFilePath} with version: ${JSON.parse(fs.readFileSync(manifestFilepath, 'utf8')).version}`);
    });
}
function packCrx(args) {
    return new Promise((resolve, reject) => {
        ...
        crx
            ...
            fs.writeFileSync(args.crxFilePath, crxBuffer);
            resolve();
        })
            .catch((err) => {
            console.error(err);
